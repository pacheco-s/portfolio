...
int main (int argc, char **argv){
    ...
    if(lin.num_inputs < 2)
        ...
        if(! load_wav(&auds[0], lin.inputs[0])){  // carrega primeiro arquivo do mix
        ...
        for(i = 1; i < (lin.num_inputs) ; i++){
            if(! load_wav(&auds[i], lin.inputs[i])){
                ...
                if(! mix_wavs(&auds[i - 1], &auds[i])){ // esta função faz um mix do 1° e 2° elemento e armazena no 1°
    ...
    finaliza_options(&lin); // fecha arquivos abertos, e libera memoria possivelmente alocada para os inputs
    return 1;
}
